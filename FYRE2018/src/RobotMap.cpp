...
std::shared_ptr<frc::Servo> RobotMap::cubeHolderStringServo;
std::shared_ptr<frc::AnalogInput> RobotMap::cameraDeleteMe;
...
void RobotMap::init() {
    ...
    liftLiftMotor.reset(new frc::Spark(6));
    lw->AddActuator("Lift", "Lift Motor", std::static_pointer_cast<frc::Spark>(liftLiftMotor));
    
    liftDistanceSensor.reset(new frc::AnalogInput(5));
    lw->AddSensor("Lift", "Distance Sensor", liftDistanceSensor);
    
    cubeHolderStringServo.reset(new frc::Servo(7));
    lw->AddActuator("CubeHolder", "StringServo", cubeHolderStringServo);
    
    cameraDeleteMe.reset(new frc::AnalogInput(0));
    lw->AddSensor("Camera", "Delete Me", cameraDeleteMe);
